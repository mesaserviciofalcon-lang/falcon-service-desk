...
export default async function MetricasVulnerabilidadFinca({ finca }: { finca: string }) {
  const rango = rangoMes(mesActualBogota());
  const where = { eai: finca, fecha: { gte: rango.inicio, lt: rango.fin } };
  const [total, abiertos, cerrados, pendientesOchoDias, porActo, detalles] = await Promise.all([
    ...
    prisma.vulnerabilidadInforme.count({ where: { eai: finca, estado: { not: "CERRADO" }, fecha: { lte: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000) } } }),
