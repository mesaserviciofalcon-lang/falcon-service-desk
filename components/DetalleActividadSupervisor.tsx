...
export default function DetalleActividadSupervisor({
  ...
  actividad: { id: number; fechaPlaneada: string; finca: string; actividad: string; area: string | null; supervisorNombre: string | null; estado: string; observacionesCierre: string | null; evidencias: Archivo[] };
  puedeCerrar: boolean;
}) {
  ...
  return <div className="mx-auto max-w-3xl space-y-5">
    ...
    {terminado ? <div className="rounded-xl border border-green-200 bg-green-50 p-5"><h2 className="font-bold text-green-950">Gestión cerrada</h2><p className="mt-2 whitespace-pre-wrap">{actividad.observacionesCierre}</p><div className="mt-4 flex flex-col gap-2">{evidencias.map((archivo) => <a key={archivo.url} href={archivo.url} target="_blank" rel="noreferrer" className="rounded border bg-white p-3 text-[#0F3D1F] underline">{archivo.nombre}</a>)}</div></div> : puedeCerrar ? <div className="rounded-xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-bold text-[#0F3D1F]">Cerrar actividad</h2><textarea value={observaciones} onChange={(event) => setObservaciones(event.target.value)} className="mt-4 min-h-32 w-full rounded-lg border p-3" placeholder="Describa la gestión realizada" /><div className="mt-4"><p className="mb-2 font-semibold">Evidencia de cierre</p><UploadButton allowedExtensions={["jpg", "jpeg", "png", "webp", "pdf"]} allowedExtensionsLabel="imágenes o PDF" onCompleteMany={(archivos: Archivo[]) => setEvidencias((actual) => [...actual, ...archivos])} /></div>{evidencias.length > 0 && <div className="mt-4 space-y-2">{evidencias.map((archivo) => <p key={archivo.url} className="rounded border bg-slate-50 p-2 text-sm">{archivo.nombre}</p>)}</div>}<button disabled={cerrando} onClick={cerrar} className="mt-5 rounded-lg bg-[#0F3D1F] px-4 py-3 font-semibold text-white disabled:bg-slate-400">{cerrando ? "Cerrando..." : "Cerrar actividad"}</button></div> : <div className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-amber-950">Esta actividad está pendiente de gestión por el supervisor asignado.</div>}
  </div>;
}
