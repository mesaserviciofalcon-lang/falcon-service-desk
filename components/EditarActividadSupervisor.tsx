...
export default function EditarActividadSupervisor({ actividad, supervisores, catalogos }: { actividad: { id: number; fechaPlaneada: string; finca: string; actividad: string; area: string | null; supervisorCorreo: string | null }; supervisores: Array<{ nombre: string; email: string }>; catalogos: Array<{ tipo: string; valor: string }> }) {
  const [guardando, setGuardando] = useState(false);
  const fincas = catalogos.filter((item) => item.tipo === "FINCA").map((item) => item.valor);
  const actividades = catalogos.filter((item) => item.tipo === "ACTIVIDAD").map((item) => item.valor);
