...
function TablaSimulacros({ simulacros, esAnalista }: { simulacros: SimulacroFila[]; esAnalista: boolean }) {
  return <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left"><tr><th className="p-3">Consecutivo</th><th className="p-3">Fecha</th><th className="p-3">Simulacro</th><th className="p-3">Resultado</th><th className="p-3">Acciones</th></tr></thead><tbody>{simulacros.map((simulacro) => <tr key={simulacro.id} className="border-t"><td className="p-3 font-semibold">{simulacro.consecutivo || `SIM-${simulacro.id}`}</td><td className="p-3">{simulacro.createdAt.toLocaleDateString("es-CO", { timeZone: "America/Bogota" })}</td><td className="p-3">{simulacro.tipo}</td><td className="p-3"><p>{simulacro.resultado}</p>{simulacro.requiereSac && !simulacro.solicitudAccion && <span className="mt-1 inline-block rounded bg-red-100 px-2 py-1 text-xs font-bold text-red-800">SAC PENDIENTE</span>}{simulacro.solicitudAccion?.estado === "EN_SEGUIMIENTO" && <span className="mt-1 inline-block rounded bg-amber-100 px-2 py-1 text-xs font-bold text-amber-800">SAC EN SEGUIMIENTO</span>}</td><td className="p-3"><div className="flex flex-wrap gap-2"><Link className="rounded bg-[#0F3D1F] px-3 py-2 font-semibold text-white" href={`/simulacros/${simulacro.id}`}>Ver detalle de simulacro</Link><a className="rounded border px-3 py-2 font-semibold" href={`/api/simulacros/${simulacro.id}/pdf`} target="_blank" rel="noreferrer">Ver PDF simulacro</a>{simulacro.solicitudAccion ? <><Link className="rounded bg-[#0F3D1F] px-3 py-2 font-semibold text-white" href={`/solicitudes-accion/${simulacro.id}`}>Ver detalle de la SAC</Link><a className="rounded border px-3 py-2 font-semibold" href={`/api/simulacros/${simulacro.id}/sac/pdf`} target="_blank" rel="noreferrer">Ver PDF SAC</a></> : esAnalista && simulacro.requiereSac ? <Link className="rounded bg-amber-500 px-3 py-2 font-semibold text-white" href={`/solicitudes-accion/${simulacro.id}`}>Crear SAC</Link> : null}</div></td></tr>)}</tbody></table></div>;
}
