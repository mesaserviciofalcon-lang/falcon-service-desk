...
function clave(anio: number, fecha: Date) {
  const mes = Number(new Intl.DateTimeFormat("en-CA", { month: "numeric", timeZone: "America/Bogota" }).format(fecha));
  return `${anio}-${mes}`;
}
...
function estadoSac(sac: { estado: string; actividadReprogramadaId: number | null; fechaReprogramacion: Date | null; fechaCierre: Date | null } | null) {
...
export default async function MetricasSimulacrosPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const inicio = new Date(Date.UTC(anio, 0, 1, 5));
  const fin = new Date(Date.UTC(anio + 1, 0, 1, 5));

  const programados = await prisma.actividadSupervisor.findMany({
    where: { actividad: { in: [...tiposSimulacro] }, fechaPlaneada: { gte: inicio, lt: fin } },
    select: { id: true, fechaPlaneada: true, finca: true, actividad: true },
  });
  ...
  for (const actividad of programados) {
    const llave = `${clave(anio, actividad.fechaPlaneada)}-${actividad.actividad}`;
    const conteo = resumenMesTipo.get(llave) || { programados: 0, detectados: 0, perdidos: 0 };
    ...
    resumenMesTipo.set(llave, conteo);
  }

  const resumenFinca = new Map<string, Conteo>();
  for (const actividad of programados) {
    const conteo = resumenFinca.get(actividad.finca) || { programados: 0, detectados: 0, perdidos: 0 };
    conteo.programados += 1;
    const informe = informesPorActividad.get(actividad.id);
    if (informe?.resultado === "DETECTADO") conteo.detectados += 1;
    if (informe && esPerdido(informe.resultado)) conteo.perdidos += 1;
    resumenFinca.set(actividad.finca, conteo);
  }
  ...
  const sacPendientes = perdidos.filter(({ informe }) => !informe.solicitudAccion).length;
  const sacsEnSeguimiento = perdidos.filter(({ informe }) => informe.solicitudAccion?.estado === "EN_SEGUIMIENTO").length;

  return <main className="mx-auto max-w-7xl space-y-6 p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold text-[#0F3D1F]">Métricas de simulacros</h1><p className="mt-1 text-slate-600">Programación, detección, pérdidas y trazabilidad de SAC por finca.</p></div><form action="/simulacros/metricas" className="flex items-end gap-2 rounded-xl border bg-white p-3 shadow-sm"><label className="flex flex-col gap-1 text-sm font-semibold text-slate-700">Año<input name="anio" type="number" min="2020" max={String(Number(partesColombia().year) + 1)} defaultValue={anio} className="w-28 rounded-lg border p-2 font-normal" /></label><button className="rounded-lg bg-[#0F3D1F] px-4 py-2 font-semibold text-white">Ver métricas</button></form></div>
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5"><Tarjeta etiqueta="Simulacros programados" valor={programados.length} /><Tarjeta etiqueta="Informes generados" valor={informes.length} color="text-[#0F3D1F]" /><Tarjeta etiqueta="Detectados" valor={informes.filter((informe) => informe.resultado === "DETECTADO").length} color="text-green-700" /><Tarjeta etiqueta="Perdidos / no detectados" valor={perdidos.length} color="text-red-700" /><Tarjeta etiqueta="SAC pendientes" valor={sacPendientes} color="text-red-700" /></section>
    <section className="rounded-xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-bold text-[#0F3D1F]">Cumplimiento mes a mes por tipo de simulacro</h2><p className="mt-1 text-sm text-slate-500">Cada celda muestra: <strong>programados / detectados / perdidos</strong>.</p><div className="mt-5 overflow-x-auto"><table className="min-w-full text-sm"><thead className="border-b bg-slate-50 text-left"><tr><th className="p-3">Mes</th><th className="p-3">Tipo</th><th className="p-3 text-center">Programados</th><th className="p-3 text-center">Detectados</th><th className="p-3 text-center">Perdidos</th></tr></thead><tbody>{Array.from({ length: 12 }, (_, indice) => indice + 1).flatMap((mes) => [...tiposSimulacro].map((tipo, indice) => { const conteo = resumenMesTipo.get(`${anio}-${mes}-${tipo}`) || { programados: 0, detectados: 0, perdidos: 0 }; return <tr key={`${mes}-${tipo}`} className="border-b last:border-0"><td className="p-3 capitalize">{indice === 0 ? nombreMes(anio, mes) : ""}</td><td className="p-3 font-medium">{etiquetaTipo(tipo)}</td><td className="p-3 text-center">{conteo.programados}</td><td className="p-3 text-center font-bold text-green-700">{conteo.detectados}</td><td className="p-3 text-center font-bold text-red-700">{conteo.perdidos}</td></tr>; }))}</tbody></table></div></section>
    <div className="grid gap-6 xl:grid-cols-2"><section className="rounded-xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-bold text-[#0F3D1F]">Ranking de fincas</h2><p className="mt-1 text-sm text-slate-500">La tasa compara los simulacros perdidos frente a los programados.</p><div className="mt-5 overflow-x-auto"><table className="min-w-full text-sm"><thead className="border-b bg-slate-50 text-left"><tr><th className="p-3">Finca</th><th className="p-3 text-center">Programados</th><th className="p-3 text-center">Detectados</th><th className="p-3 text-center">Perdidos</th><th className="p-3 text-center">Tasa pérdida</th></tr></thead><tbody>{rankingFincas.map((fila) => <tr key={fila.finca} className="border-b last:border-0"><td className="p-3 font-semibold">{fila.finca}</td><td className="p-3 text-center">{fila.programados}</td><td className="p-3 text-center text-green-700">{fila.detectados}</td><td className="p-3 text-center font-bold text-red-700">{fila.perdidos}</td><td className="p-3 text-center">{fila.porcentajePerdida}%</td></tr>)}{rankingFincas.length === 0 && <tr><td colSpan={5} className="p-5 text-center text-slate-500">No hay simulacros programados en {anio}.</td></tr>}</tbody></table></div></section><section className="space-y-4 rounded-xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-bold text-[#0F3D1F]">Lectura de resultados</h2><Indicador titulo="Finca con más pérdidas" valor={fincaMasPierde ? `${fincaMasPierde.finca} · ${fincaMasPierde.perdidos} perdido(s)` : "Sin datos"} clase="border-red-200 bg-red-50 text-red-900" /><Indicador titulo="Finca más alineada con Seguridad" valor={fincaMasAlineada ? `${fincaMasAlineada.finca} · ${fincaMasAlineada.porcentajePerdida}% de pérdida` : "Sin datos"} clase="border-green-200 bg-green-50 text-green-900" /><Indicador titulo="SAC en seguimiento" valor={`${sacsEnSeguimiento} SAC`} clase="border-amber-200 bg-amber-50 text-amber-900" /><p className="text-xs text-slate-500">La finca más alineada se define por la menor tasa de pérdida entre sus simulacros programados en el año seleccionado.</p></section></div>
    <section className="rounded-xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold text-[#0F3D1F]">Simulacros perdidos y trazabilidad de la SAC</h2><p className="mt-1 text-sm text-slate-500">Identifica finca, tipo de simulacro y el estado de la acción correctiva o reprogramación.</p></div><Link href="/simulacros" className="rounded border px-4 py-2 font-semibold">Ver simulacros</Link></div><div className="mt-5 overflow-x-auto"><table className="min-w-full text-sm"><thead className="border-b bg-slate-50 text-left"><tr><th className="p-3">Fecha</th><th className="p-3">Finca</th><th className="p-3">Tipo</th><th className="p-3">Consecutivo</th><th className="p-3">Estado SAC / reprogramación</th><th className="p-3">Acción</th></tr></thead><tbody>{perdidos.map(({ informe, actividad }) => { const estado = estadoSac(informe.solicitudAccion); return <tr key={informe.id} className="border-b last:border-0"><td className="p-3">{actividad.fechaPlaneada.toLocaleDateString("es-CO", { timeZone: "America/Bogota" })}</td><td className="p-3 font-semibold">{actividad.finca}</td><td className="p-3">{etiquetaTipo(informe.tipo)}</td><td className="p-3">{informe.consecutivo || `SIM-${informe.id}`}</td><td className="p-3"><span className={`inline-block rounded px-2 py-1 text-xs font-bold ${estado.clase}`}>{estado.texto}</span></td><td className="p-3"><Link href={`/simulacros/${informe.id}`} className="font-semibold text-[#0F3D1F] underline">Ver detalle</Link></td></tr>; })}{perdidos.length === 0 && <tr><td colSpan={6} className="p-5 text-center text-slate-500">No hay simulacros perdidos en {anio}.</td></tr>}</tbody></table></div></section>
  </main>;
}
