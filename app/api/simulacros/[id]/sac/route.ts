...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!descripcionSituacion || !analisisCausa || factoresCausa.length === 0 || planAccion.length === 0) return Response.json({ error: "Complete la descripción, análisis de causa, factores y plan de acción" }, { status: 400 });
    const consecutivo = `SAC-${simulacro.finca}-${String(simulacro.id).padStart(4, "0")}`;
    const sac = await prisma.solicitudAccion.create({ data: {
      ...
      comentariosCierre: texto(body.comentariosCierre) || null, analistaNombre: session.user.name || session.user.email, analistaCorreo: session.user.email, fechaCierre: new Date(),
    } });
    ...
    if (correosJefe.length) await enviarCorreo({
      ...
      html: `<p>Se informa el cierre de la Solicitud de Acción <strong>${consecutivo}</strong>, originada por el simulacro de ${simulacro.tipo} en la finca ${simulacro.finca}.</p><p><a href="${getAppUrl()}/api/simulacros/${simulacro.id}/sac/pdf">Ver PDF de la SAC</a></p>`,
