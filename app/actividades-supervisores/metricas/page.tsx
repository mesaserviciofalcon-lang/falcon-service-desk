...
export default async function MetricasActividadesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const supervisoresOrganigrama = await prisma.equipoAdministrativo.findMany({
    where: { cargo: { contains: "SUPERVISOR", mode: "insensitive" } },
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2"><section className="rounded-xl border bg-white p-5"><h2 className="text-xl font-bold text-[#0F3D1F]">Cumplimiento por supervisor</h2><p className="mt-1 capitalize text-sm text-slate-500">{rango.etiquetaMes}</p><div className="mt-5 space-y-5">{porSupervisor.map(([nombre, fila]) => { const porcentaje = fila.total ? Math.round((fila.cumplidas / fila.total) * 100) : 0; const nombreNormalizado = normalizarNombre(nombre); const foto = supervisoresOrganigrama.find((supervisor) => { const nombreSupervisor = normalizarNombre(`${supervisor.nombres} ${supervisor.apellidos}`); return nombreSupervisor.includes(nombreNormalizado) || nombreNormalizado.includes(nombreSupervisor); })?.fotoUrl; return <div key={nombre}><div className="mb-1 flex justify-between gap-3 text-sm"><span className="flex items-center gap-2 font-semibold"><span className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#0F3D1F] text-xs text-white">{foto ? <Image src={foto} alt={`Foto de ${nombre}`} fill unoptimized sizes="40px" className="object-cover" /> : iniciales(nombre)}</span>{nombre}</span><span>{fila.cumplidas}/{fila.total} cumplidas · {porcentaje}%</span></div><div className="h-4 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-[#2FAE4A]" style={{ width: `${porcentaje}%` }} /></div><p className="mt-1 text-xs text-slate-500">Ejecutadas: {fila.terminadas} · Pendientes: {fila.total - fila.terminadas}</p></div>; })}{porSupervisor.length === 0 && <p className="text-slate-500">No hay actividades para este mes.</p>}</div></section>
