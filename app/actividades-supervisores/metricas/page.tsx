...
function coincidenNombres(nombreActividad: string, nombreEquipo: string) {
  const actividad = String(nombreActividad || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toUpperCase().split(/\s+/).filter(Boolean);
  const equipo = String(nombreEquipo || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toUpperCase().split(/\s+/).filter(Boolean);
  if (!actividad.length || !equipo.length || actividad[0] !== equipo[0]) return false;
  if (actividad.length === 1 || equipo.length === 1) return true;
  return actividad.slice(1).some((parte) => equipo.slice(1).some((apellido) => parte.startsWith(apellido.slice(0, 5)) || apellido.startsWith(parte.slice(0, 5))));
}
...
function claseBarraCumplimiento(porcentaje: number, total: number) {
  if (!total) return "bg-slate-300";
  if (porcentaje === 0) return "bg-amber-400";
  if (porcentaje < 70) return "bg-red-600";
  return "bg-green-600";
}
...
export default async function MetricasActividadesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const supervisoresOrganigrama = await prisma.equipoAdministrativo.findMany({
    select: { nombres: true, apellidos: true, fotoUrl: true },
  });
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold text-[#0F3D1F]">Cumplimiento mensual del departamento</h2><p className="mt-1 text-sm text-slate-500">Porcentaje de actividades cumplidas en la fecha planeada durante {rango.year}.</p></div><span className="rounded-full bg-green-100 px-3 py-1 text-sm font-semibold text-green-800">General anual: {anual.cumplimiento}%</span></div><div className="mt-4 flex h-72 items-end gap-2 overflow-x-auto border-b border-slate-200 pb-1">{meses.map((fila) => { const porcentaje = fila.total ? Math.round((fila.cumplidas / fila.total) * 100) : 0; return <div key={fila.month} className="flex h-full min-w-14 flex-1 flex-col items-center justify-end gap-2"><span className="text-xs font-bold text-[#0F3D1F]">{fila.total ? `${porcentaje}%` : "—"}</span><div className="flex h-56 items-end"><div title={`${etiquetaMesCorto(rango.year, fila.month)}: ${fila.cumplidas}/${fila.total} cumplidas (${porcentaje}%)`} className={`flex w-10 items-start justify-center rounded-t-md pt-1 text-xs font-bold text-white ${claseBarraCumplimiento(porcentaje, fila.total)}`} style={{ height: fila.total ? `${Math.max(8, porcentaje)}%` : "3px" }}>{fila.total ? fila.cumplidas : ""}</div></div><span className="text-xs font-bold text-slate-600">{etiquetaMesCorto(rango.year, fila.month)}</span></div>; })}</div><p className="mt-3 text-xs text-slate-500">Verde: 70% o más. Rojo: menos de 70%. Amarillo: sin cumplimiento. El número dentro de la barra corresponde a las actividades cumplidas en fecha.</p></section>
    <div className="grid gap-6 xl:grid-cols-2"><section className="rounded-xl border bg-white p-5"><h2 className="text-xl font-bold text-[#0F3D1F]">Cumplimiento por supervisor</h2><p className="mt-1 capitalize text-sm text-slate-500">{rango.etiquetaMes}</p><div className="mt-5 space-y-5">{porSupervisor.map(([nombre, fila]) => { const porcentaje = fila.total ? Math.round((fila.cumplidas / fila.total) * 100) : 0; const foto = supervisoresOrganigrama.find((supervisor) => coincidenNombres(nombre, `${supervisor.nombres} ${supervisor.apellidos}`))?.fotoUrl; return <div key={nombre}><div className="mb-1 flex justify-between gap-3 text-sm"><span className="flex items-center gap-2 font-semibold"><span className="relative flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#0F3D1F] text-xs text-white">{foto ? <Image src={foto} alt={`Foto de ${nombre}`} fill unoptimized sizes="48px" className="object-cover" /> : iniciales(nombre)}</span>{nombre}</span><span>{fila.cumplidas}/{fila.total} cumplidas · {porcentaje}%</span></div><div className="h-4 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-[#2FAE4A]" style={{ width: `${porcentaje}%` }} /></div><p className="mt-1 text-xs text-slate-500">Ejecutadas: {fila.terminadas} · Pendientes: {fila.total - fila.terminadas}</p></div>; })}{porSupervisor.length === 0 && <p className="text-slate-500">No hay actividades para este mes.</p>}</div></section>
